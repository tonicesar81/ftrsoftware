...
require_once app_path('Classes/setasign/Fpdf/fpdf.php');
require_once app_path('Classes/setasign/Fpdi/autoload.php');
...
class PDFWatermark {
	 ...
	 * @return void
	 */
	function __construct($file, $pdf = null) {
		...
		switch( $imagetype ) {
			...
			case IMAGETYPE_PNG:
                                if(is_null($this->_source)){
                                    ...
                                    //if stretch
                                        $pdf = new Fpdi();
                                        $pdf->setSourceFile($this->_source);
                                        $tplIdx = $pdf->importPage(1);
                                        $templateDimension = $pdf->getTemplateSize($tplIdx);
                                        $pdf->AddPage();
    //                                    $x = (round($templateDimension['width']) / 96) * 25.4;
    //                                    $y = (round($templateDimension['height']) / 96) * 25.4;
                                        //pixels = (mm * dpi) / 25.4
                                        $x = ($templateDimension['width'] * 96) / 25.4;
                                        $y = ($templateDimension['height'] * 96) / 25.4;
                                        $dst = ImageCreateTrueColor($x,$y);
                                        imageAlphaBlending($dst, false);
                                        imagesavealpha($dst,true);
                                        ImageCopyResampled($dst,$image,0,0,0,0,$x,$y,$width,$height);
    //                                    ImageCopyResampled($dst,$image,0,0,0,0,$templateDimension['width'],$templateDimension['height'],$width,$height);
                                        //exit($templateDimension['width'].'('.$x.') ,'.$templateDimension['height'].'('.$y.')');
                                        //imagescale($image,round($templateDimension['width']));
                                    //endif
                                    imagepng($image, $path);
                                    imagepng($dst, $path);
                                    imagedestroy($image);
