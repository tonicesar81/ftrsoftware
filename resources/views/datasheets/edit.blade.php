...
<div class="container">
    ...
    <div class="card">
        ...
        <div class="card-body">
            {!! Form::open(['action' => ['DatasheetsController@update', $datasheet->id], 'method' => 'put']) !!}
            <div class="form-row">
                <div class="form-group col-md-4">
                    {!! Form::label('shoppings_id', 'Shopping') !!}
                    {!! Form::select("shoppings_id",$shopping ,$datasheet->shoppings_id, ["class" => "form-control mb-2 selectable"]) !!}
                </div>
                <div class="form-group col-md-4">
                    {!! Form::label('loja', 'Nome da loja') !!}
                    {!! Form::text('loja', $datasheet->loja, ['class' => 'form-control']); !!}
                </div>
                <div class="form-group col-md-4">
                    {!! Form::label('numero', 'Número de identificação') !!}
                    {!! Form::text('numero', $datasheet->numero, ['class' => 'form-control']); !!}
                </div>
                ...
                <div id="equips" class="form-group col-12 row">
                    ...
                    <div class="form-group col-12 row">
                        <div class="form-group col-md-2">
                            ...
                            {!! Form::number('quantidade[]', $detalhe->quantidade, ['class' => 'form-control']); !!}
                        </div>
                        <div class="form-group col-md-3">
                            {!! Form::label('dsnomes_id', 'Equipamento') !!}
                            {!! Form::select('dsnomes_id[]',$nome ,$detalhe->dsnomes_id, ['class' => 'form-control selectable']) !!}
                        </div>
                        <div class="form-group col-md-3">
                            {!! Form::label('dstipos_id', 'Tipo') !!}
                            {!! Form::select('dstipos_id[]',$tipo ,$detalhe->dstipos_id, ['class' => 'form-control selectable']) !!}
                        </div>
                        <div class="form-group col-md-3">
                            {!! Form::label('dslocais_id', 'Localidade') !!}
                            {!! Form::select('dslocais_id[]',$local ,$detalhe->dslocais_id, ['class' => 'form-control selectable']) !!}
                        </div>
                        {!! Form::button("x", ["class" => "btn btn-sm btn-outline-info", "onClick" => "$(this).parent().remove();" ]); !!}
                    </div>
                    @endforeach
                </div>
